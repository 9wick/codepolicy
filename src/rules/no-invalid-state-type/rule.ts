...
const definition: RuleDefinition = {
  meta: { scope: ['type', 'interface'] },
  create: () =>
    okAsync((ctx) =>
      ctx.llm.judge({
        criteria: `以下の type / interface 定義が「no invalid state type」の原則に違反していないかを検証してください。
...
## 違反として扱わないもの
- 単に primitive を使っているだけで、意味の取り違えが現実的に起きない場合
- 未確定状態を表す型自体（入力途中・検証前・正規化前の型として閉じ込められていれば違反ではない）
...
\`\`\`
検証前の入力を表す未確定状態の型として閉じ込められているためPASSする

\`\`\`typescript
type RetryCount = number;
\`\`\`
primitive のままでも意味の取り違えが現実的に起きないためPASSする`,
